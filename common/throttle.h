...
public:
    ...
    int consume(uint64_t amount, Priority prio = Priority::High) {
        ...
        if (unlikely(amount > m_limit && m_limit > 0)) {
            // LOG_WARN("throttle: amount ` too large, fall back to starving mode and sleep ` time window");
            uint64_t starving_limit = m_limit * 100 / starving_percent;
            uint64_t n = amount / starving_limit;
            photon::thread_usleep(m_time_window * n);
            return 0;
        }
